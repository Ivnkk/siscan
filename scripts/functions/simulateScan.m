...
%         sigma = 8*log(2)/(tau^2);
        tg  = tau/(2*log(2));
        Ew = exp(-((wf-w0).*tg).^2).*exp(1i.*phase);
%         Ew = sqrt(exp(-((wf-w0).^2)./sigma)).*exp(1i.*phase);
    case 'sech'
        ts = tau/1.763;
        Ew = sqrt(sech(pi.*(wf-w0).*ts./2).^2).*exp(1i.*phase);
        
end

%phase matrix
wf = linspace(wf(1)-1,wf(end)+1,length(wf)); % extend the frequency vector to make sure that the 
